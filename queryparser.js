...
function queryParser(query){
    ...
    querywords = query.replace(/\s+/g," ").
        replace(/((?:[a-zA-ZõäöüšžÕÄÖÜŠŽ\?\*][a-zA-ZõäöüšžÕÄÖÜŠŽ\?\*\-\.0-9]*\:?)|["'])/g,"\t$1\t").split("\t");
    ...
    for(i=0, len = words.length; i<len; i++){
        word = words[i]
        if(word && word.length>1 && word.substr(-1) == ":" && i < len - 1 && words[i+1].match(/^[a-zA-ZöäõüšžÕÄÖÜŠŽ0-9\-\.]+$/)){
    ...
    for(i=0, len = words.length; i<len; i++){
        if(words[i].match(/^[a-zA-ZöäõüšžÕÄÖÜŠŽ0-9]/) && !words[i].match(/[\?\*\:]/) && skiplist.indexOf(words[i].toLowerCase())<0){
            if(words[i].match(/^[A-ZÕÄÖÜŠŽ0-9\-]+$/)){
                txtcase = "allcaps";
